...
dk_dT= -4*10^(-5); %Förändringen i k med hänseende på temeperaturen[K^(-1)]
dk_dalpha=(-100)*10^(-5); %Föändringen i k med hänseende på voiden [% void]
...
%Tidssteg
t = linspace(0, 1, 1000); %tidsvektorn
x=1/1000; %[s] tidssteg mellan varje iteration
...
for i=1:length(t)
    if i==1 
        ...
        dT_dt(i) = (P(i)-P(1)) * x / (m_UO2 * c_p); %hur bränsletemperatur ändras över tid
        T(i) = T(i-1) + dT_dt(i); %den nya bränsletemperaturen [K]

        rho_steam(i) = XSteam('rho_pT', 70, (T(i)-273))*10^(-3); %ångans densitet för olika T, tryck konstant [kg/cm^3]
        delta_h_steam(i) = (XSteam('h_pT', 70, (T(i)-273))-XSteam('h_pT', 70, 700))*10^3; %skillnad i entalpi [J/kg]
        P_kapsling(i)=(T(i-1)-(700+273)) * U_H20 * (1-alpha(i-1)); %effekt från kapslingen [W/cm]
        dalpha_dt(i)=(((P_kapsling(i)-P_mod)/delta_h_steam(i))/rho_steam(i))/V_mod; %förändring i void under tid
        alpha(i) = alpha(i-1) + dalpha_dt(i); %den nya voiden

        dk_dt(i) = (dk_dalpha * dalpha_dt(i))+(dk_dT * dT_dt(i)); %förändring i k-värde över tid
        k(i) = k(i-1) + dk_dt(i); %det nya k-värdet.
    end
end
